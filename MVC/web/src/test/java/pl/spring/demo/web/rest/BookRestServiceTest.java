   ...
   @Test
    public void testShouldDeleteBook() throws Exception {
        // given
   		...
   		String json = FileUtils.readFileToString(file);
    	final Long id = 1L;
    	final BookTo bookTo1 = new BookTo(id, "testTitle", "Author1");
		Mockito.when(bookService.deleteBook2(bookTo1)).thenReturn(bookTo1);
        // when
        ResultActions response = this.mockMvc.perform(delete("/book")
        		...
        		.content(json.getBytes()));
        // then
        Mockito.verify(bookService).deleteBook2(bookTo1);
        response.andExpect(status().isOk())
